...
uniqueHosts = hosts.map(lambda x: (x,1)).reduceByKey(lambda a, b: a+b).cache()
...
dayHostCount = dayGroupedHosts.map(lambda (a,b): (a,1)).reduceByKey(lambda a, b: a + b)
...
groupedByDay = dayAndHostTuple.map(lambda (x,y): (x,1)).reduceByKey(lambda a, b: a + b)
...
errHostsTop25 = errHostsSum.takeOrdered(25, lambda (x,y): -y)
