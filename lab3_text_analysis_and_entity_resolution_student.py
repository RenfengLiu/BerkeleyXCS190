...
def simpleTokenize(string):
    ...
    """
    l = re.split(split_regex,string, flags=re.IGNORECASE)
    l2 = [x.lower()  for x in l if len(x)]
#     l2 = map(lambda x:  x.lower(), l)
    return l2
...
def tokenize(string):
    ...
    l = simpleTokenize(string)
    l2 = [x for x in l if x not in stopwords]
    return  l2
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.takeOrdered(1, lambda (recordId, tokens): -1 * len(tokens))
...
def idfs(corpus):
    ...
    """
    N = corpus.count()
    uniqueTokens = corpus.flatMap(lambda (x,y): tf(y).keys())
...
crossSmall = (googleSmall
              .map(lambda (x, y): (1, (x,y)))
              .join(amazonSmall.map(lambda (x, y):(1,(x,y))))
              .map(lambda (k,v): v)
              .cache())
...
# TODO: Replace <FILL IN> with appropriate code
sims = similaritiesBroadcast.map(lambda (x,y,c): (y+' '+x, c))
# print sims.count()
