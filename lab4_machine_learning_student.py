...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    avg = sum(IDandRatingsTuple[1])/float(num)
    return (ID, (num, avg))
...
movieNameWithAvgRatingsRDD = (moviesRDD
                             ...
                             .map(lambda (ID,(name, rating)): (rating[1], name, rating[0])))
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        ...
                        .map(lambda ((uid, mid),(r1, r2)): (r1-r2)*(r1-r2))
                       )
...
myModel = ALS.train(trainingRDD,bestRank, seed=seed, iterations=iterations,
                      lambda_=regularizationParameter)
testForPredictingRDD = testRDD.map(lambda (uid, mid, r): (uid, mid))
...
# TODO: Replace <FILL IN> with appropriate code
# ratingsRDD contains tuples of the form (UserID, MovieID, Rating).
trainingAvgRating = trainingRDD.map(lambda x: x[2]).reduce(lambda a,b: a+b)/float(trainingRDD.count())
...
myUnratedMoviesRDD = (moviesRDD
                      .join( myRatingsRDD.map(lambda (uid, mid, r): (mid, r)))
#                       .filter(lambda (mid,(v1,v2)): v2==None)
                      .map(lambda (mid,(v1,v2)): (myUserID, mid))
                     )
...
predictedRDD = predictedRatingsRDD.map(lambda (uid, mid, r): (mid, r))
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .filter(lambda (mid, (r, n)): n>75)
                       .join(moviesRDD)
                       .map(lambda (mid, ( (r,n), t )): (r, t, n) )
                      )
