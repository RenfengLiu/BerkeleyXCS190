...
parsedSamplePoints = map(parsePoint, samplePoints) #[ parsePoint(x) for x in samplePoints ]#samplePoints.map(parsePoint)
# print parsedSamplePoints[0]
...
def calcRMSE(labelsAndPreds):
    ...
    """
    return math.sqrt(labelsAndPreds.map(lambda (x, y): squaredError(x, y)).reduce(lambda a, b: a + b)/float(labelsAndPreds.count()))
...
def twoWayInteractions(lp):
    ...
    """
    label = lp.label
    f = lp.features.toArray().tolist()
    fnew = list(itertools.product(f, f))
    fnew = [ x*y for (x,y) in fnew]
    fnew = np.array(fnew)
    fnew = np.hstack((f, fnew))
    
    return LabeledPoint(label, fnew)
